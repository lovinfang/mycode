 ...
 */
//@RestController  返回的是json字符串，不适合返回是页面名
public class JackController {
    private static final Logger logger = LoggerFactory.getLogger(JackController.class);
    
    @Value("${application.hello:Hello jack}")  //application.hello 如果配置则取配置的值，如果没有配置，则取 Hello jack
    ...
    @RequestMapping(value = {"/", "/index"})  //http://localhost:8080/     这个是Spring的Servlet
    public String index(Map<String, Object> model) {
        // 直接返回字符串，框架默认会去 spring.view.prefix 目录下的 （index拼接spring.view.suffix）页面
        // 本例为 /WEB-INF/jsp/index.jsp
        model.put("time", new Date());
        model.put("message", this.hello);
        return "index";  //返回 index.jsp 页面路径配置 spring.mvc.view.prefix=/WEB-INF/jsp/
    }
